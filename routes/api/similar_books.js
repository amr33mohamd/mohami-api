app.get('/api/similar_books',function(req,res){
    ...
    con.query('SELECT name, author_name FROM books WHERE id=? LIMIT 1', [book_id], function(err,bookData) {
        if(!err) {
            var correctQueries = [
                "%" + bookData[0].name + "%",
            ...
            con.query(`SELECT id, name AS book_name, image AS book_photo, author_name,
                category_id FROM books WHERE (name LIKE ?) OR (author_name LIKE ?) OR (descc LIKE ?) LIMIT 5`,
                [correctQueries[0], correctQueries[1], correctQueries[0]],
